...
const minOf = function (numbers) {
  return numbers.reduce((a, b) => a > b ? b : a, Infinity);
};
...
const addIndividualByProperty = (storer, eachPerson) => {
  storer[eachPerson.age] = [eachPerson];

  return storer;
};
const groupByProperty = function (objects) {
  return objects.reduce(addIndividualByProperty, {});
};

console.log(groupByProperty([{ name: "John", age: 25 }, { name: "Jane", age: 30 }]));
...
const items = [{ category: 'A', value: 10 }, { category: 'B', value: 20 }, { category: 'A', value: 30 }];

const getSumOfEachCategory = function (sumOfEachCategory, obj) {
  if (obj.category in sumOfEachCategory) {
    sumOfEachCategory[obj.category] += obj.value;
  ...
  // return sumOfEachCategory[obj.category] = obj.category in sumOfEachCategory ? sumOfEachCategory[obj.category] + obj.value : obj.value;
};

const sumByCategory = function (items) {
  return items.reduce(getSumOfEachCategory, {});
};

console.table(sumByCategory(items));